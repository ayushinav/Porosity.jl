@testitem "viscosity tests" tags=[:visc] begin
    ...
    outs = (
        HZK2011=RockphyViscous(
            Float32[1.0f-12, 2.0f-12, 5.0f-12, 1.2f-11, 2.8f-11, 6.2f-11,
                1.36f-10, 2.86f-10, 5.86f-10, 1.1710001f-9, 2.283f-9],
            [8.9619f18, 3.8154f18, 1.6603f18, 7.410f17, 3.397f17,
                1.601f17, 7.75f16, 3.85f16, 1.96f16, 1.03f16, 5.5f15]),
        HK2003=RockphyViscous(
            Float32[
                3.0f-11, 7.999999f-11, 1.8999999f-10, 4.5999998f-10, 1.05f-9, 2.3499998f-9,
                5.09f-9, 1.0709999f-8, 2.191f-8, 4.3609997f-8, 8.4579995f-8],
    ...
    for i in eachindex(methods_list)
        ...
        for k in fieldnames(RockphyViscous)
            @test all(isapprox.(
                log10.(getfield(out_, k)), log10.(getfield(outs[m], k)), rtol=1.0f-2),)
        end
